...
def parse_existing_article(article_path):
    ...
    return title, description, article_date, tags, featured
...
def main():
    ...
    # Strip a leading h1 so the title doesn't duplicate the article header
    markdown_raw = re.sub(r"^#[^#][^\n]*\n+", "", markdown_raw.lstrip())
    ...
    if os.path.isfile(article_path):
        ...
        ex_title, ex_description, ex_date, ex_tags, ex_featured = parse_existing_article(article_path)
        print(f"  Preserving:")
        ...
        article_js = generate_article_js(
            slug, var_name, ex_title, ex_description, ex_date, ex_tags, ex_featured, content
        )
    ...
    print(
        f"\nDone! Next steps:\n"
        f"  1. npm start   → preview at /#/articles/{slug}\n"
        f"  2. npm run build && npm run deploy   → publish\n"
    )
